...
roi_x1, roi_y1 = 360, 200  # Top-left corner
roi_x2, roi_y2 = 720, 1280  # Bottom-right corner
...
while cap.isOpened():
    ...
    roi_mask = np.zeros_like(threshold)  # Create a mask of the same size as the threshold image
    cv2.rectangle(roi_mask, (roi_x1, roi_y1), (roi_x2, roi_y2), (255, 255, 255), -1)
    threshold_in_roi = cv2.bitwise_and(threshold, roi_mask)

    dilated = cv2.dilate(threshold, (1,1), iterations=1)
    contours, _ = cv2.findContours(threshold_in_roi, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    detections = []
